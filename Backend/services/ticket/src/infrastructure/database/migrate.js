...
async function getExecutedMigrations() {
  const [rows] = await sequelize.query(`
    SELECT name FROM ticket_schema_migrations ORDER BY name ASC;
  `);
...
async function markMigrationAsExecuted(name, transaction) {
  await sequelize.query(
    `INSERT INTO ticket_schema_migrations (name) VALUES (:name);`,
    {
      replacements: { name },
      transaction
    }
...
async function runMigrations() {
  ...
  for (const file of migrationFiles) {
    ...
    console.log(`Running ticket migration: ${file}`);
    ...
    console.log(`Ticket migration completed: ${file}`);
  }

  console.log("All ticket migrations completed");
