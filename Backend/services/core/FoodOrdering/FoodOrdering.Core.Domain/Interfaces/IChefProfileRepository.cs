...
public interface IChefProfileRepository
{
    ...
    Task<IEnumerable<ChefProfile>> GetAllAsync(CancellationToken cancellation = default);
    Task AddAsync(ChefProfile profile, CancellationToken cancellation = default);
    Task UpdateAsync(ChefProfile profile, CancellationToken cancellation = default);
